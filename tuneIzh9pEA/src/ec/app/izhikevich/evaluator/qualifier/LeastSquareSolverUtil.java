...
public class LeastSquareSolverUtil {
	...
	public SolverResultsStat solveFor2Parms(double initM1, double initC1){	
		...
		LeastSquaresProblem problem = new LeastSquaresBuilder().
		  ...
		  double m = optimum.getPoint().getEntry(0);
		  double c = optimum.getPoint().getEntry(1);
		 
		  double[] points = getAbsDouble(optimum.getResiduals());
		 
		  SolverResultsStat stat = new SolverResultsStat(m,c,points);
		  stat.setSolverStats(optimum.getRMS(), optimum.getEvaluations(), optimum.getIterations());
	...
	public SolverResultsStat solveFor3Parms(double initM1, double initC1, double initC2){	
		...
		LeastSquaresProblem problem = new LeastSquaresBuilder().
		  ...
		  double m1 = optimum.getPoint().getEntry(0);
		  double c1 = optimum.getPoint().getEntry(1);
		  double c2 = optimum.getPoint().getEntry(2);
		  
		  double[] points = getAbsDouble(optimum.getResiduals());
		 
		  SolverResultsStat stat = new SolverResultsStat(m1,c1, c2, points, _3parmfn.getBreakPoint());
		  stat.setSolverStats(optimum.getRMS(), optimum.getEvaluations(), optimum.getIterations());
		  if(timer)  {
			  TimeUnit unit = TimeUnit.MILLISECONDS;
			  time += unit.convert(System.nanoTime()-timeStart, TimeUnit.NANOSECONDS);
	...
	public SolverResultsStat solveFor4Parms(double initM1, double initC1, double initM2, double initC2){	
		...
		LeastSquaresProblem problem = new LeastSquaresBuilder().
		  ...
		  double m1 = optimum.getPoint().getEntry(0);
		  double c1 = optimum.getPoint().getEntry(1);
		  double m2 = optimum.getPoint().getEntry(2);
		  double c2 = optimum.getPoint().getEntry(3);
		  
		  double[] points = getAbsDouble(optimum.getResiduals());
		 
		  SolverResultsStat stat = new SolverResultsStat(m1,c1, m2, c2, points, _4parmfn.getBreakPoint());
		  stat.setSolverStats(optimum.getRMS(), optimum.getEvaluations(), optimum.getIterations());
		  if(timer)  {
			  TimeUnit unit = TimeUnit.MILLISECONDS;
			  time += unit.convert(System.nanoTime()-timeStart, TimeUnit.NANOSECONDS);
	...
	public static void main(String[] args) {
		...
		double[] X = {276, 462};//GeneralFileReader.readDoublesSepByLine(fileName);
		fileName = "input/other_test/Y.txt";
		double[] Y = {136, 186};//GeneralFileReader.readDoublesSepByLine(fileName);
	//	GeneralUtils.displayArray(Y);
		...
		if(!StatAnalyzer.isSignificantImprovement(stat1.getFitResidualsAbs(), stat2.getFitResidualsAbs())){
		/*	System.out.println("1 - 3:");
			if(!StatAnalyzer.isSignificantImprovement(stat1.getFitResidualsAbs(), stat3.getFitResidualsAbs())){
				System.out.println("1 - 4:");
				if(!StatAnalyzer.isSignificantImprovement(stat1.getFitResidualsAbs(), stat4.getFitResidualsAbs())){
			...
			if(!StatAnalyzer.isSignificantImprovement(stat2.getFitResidualsAbs(), stat3.getFitResidualsAbs())){
				...
				if(!StatAnalyzer.isSignificantImprovement(stat3.getFitResidualsAbs(), stat4.getFitResidualsAbs())){
