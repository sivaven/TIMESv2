 ...
 */
	...
	private float[] synWeight;
	
	private CarlMcSimData carlMcSimData;
	private boolean externalSimulatorUsed;
	
	public MultiCompConstraintEvaluator(Izhikevich9pModelMC model, boolean display, float[] weight,  CarlMcSimData carlMcData) {
		this.model = model;
	...
	public MultiCompConstraintEvaluator(Izhikevich9pModelMC model, boolean display, float[] weight) {
		this.model = model;
		this.display = display;
	...
	public float calculateMcConstraintError(InputMCConstraint mcConstraintData){
		...
		if(mcConstraintData.getType().equals(MCConstraintType.EXCITABILITY)) {	
			...
			if(externalSimulatorUsed){
				...
				float current_min = (float)mcConstraintData.getAttribute(MCConstraintAttributeID.current_min);
				float current_max = (float)mcConstraintData.getAttribute(MCConstraintAttributeID.current_max);
				float current_dur = (float)mcConstraintData.getAttribute(MCConstraintAttributeID.current_duration);
				float current_step = (float)mcConstraintData.getAttribute(MCConstraintAttributeID.current_step);	
				float rheo_diff = (float)mcConstraintData.getAttribute(MCConstraintAttributeID.rheo_diff);	
	 ...
	 */
	private float dendriticExcitabilityError(float rheoDiff, float iMin, float iMax, float iDur, float iStep) {
		float error = 0;				
		model.determineRheobases(iDur, iMin, iMax, iStep);
		float[] iComp = model.getRheoBases();			
	...
	private float dendriticIRError() {
		...
		for(int i=1;i<absVs.length;i++){
			...
			if(dendVdef > somaVDef){
				error += 0;
			}else{
				error += (float) StatUtil.calculateObsNormalizedError(somaVDef, dendVdef);
			}			
			if(display) {
				String displayString = "\nIRErr(cmp#"+i+").\t("+
										"-100"
	...
	public float forwardSpikePropagationError() {
		float error = 0;	
		boolean[] spikeProped = carlMcSimData.getSpikeProped();
		for(int i=0; i<spikeProped.length;i++){
			if(spikeProped[i]){
				error+=0;
			}
			else{
			...
			if(display) {
				String displayString = "\nfrwSpkPrpErr(FrmCmp#"+(i+1)+").\t"+spikeProped[i]+"\t"+error;
				System.out.print(displayString);
			}
		}		
		return error;	
	...
	private float synapticallyStimulatedEpspError(float expEPSPmin, float expEPSPmax, float timeConst, float simDur) {
		...
		for(int i=0;i<synWeight.length;i++){
			float[] weight = new float[synWeight.length];
			...
			float somaEpsp = (float)peakVolt - model.getvR()[0];
