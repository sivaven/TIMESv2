...
public:
	 SpikePatternJson(float i, float idur, float tstep, int* spike_times, int stlength, float* vtrace, int vtlength){
		 I= i;
		 Idur = idur;
		 tStep = tstep;
...
public:
	 MultiCompDataJson( int* _rheos, float* _vDefs, bool* _spikeProps, float* _epsps, int _compCount){
		 rheos = _rheos;
		 vDefs = _vDefs;
